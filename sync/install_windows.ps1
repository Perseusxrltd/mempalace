...
<#
.SYNOPSIS
    MemPalace one-shot setup for Windows.

.DESCRIPTION
    Sets up the full MemPalace stack:
      1. Creates ~/.mempalace directory structure
...
if (-not (Test-Path "$RepoRoot\mempalace\mcp_server.py")) {
    $check = Split-Path $PSScriptRoot -Parent
    if (Test-Path "$check\mempalace\mcp_server.py") { $RepoRoot = $check }
}

$MempalDir     = "$env:USERPROFILE\.mempalace"
...
Write-Host ""
Write-Host "MemPalace Windows Setup" -ForegroundColor White
...
# ---------------------------------------------------------------------------
Write-Step "Creating ~/.mempalace directory structure"
...
$hookSrc = "$RepoRoot\hooks\mempal_save_hook.py"
$hookDst = "$MempalDir\hooks\mempal_save_hook.py"
if (Test-Path $hookSrc) {
    Copy-Item $hookSrc $hookDst -Force
    Write-Ok "Copied mempal_save_hook.py"
} else {
    Write-Warn "mempal_save_hook.py not found at $hookSrc"
}
...
if ((Test-Path $vllmSrc) -and (-not $SkipVllm)) {
    $wslMntPath = "/mnt/c/Users/$WslUser/projects/mempalace/sync/run_vllm.sh"
...
if ($SkipHook) {
    ...
    $hookCmd = "python3 `"$MempalDir\hooks\mempal_save_hook.py`""
    ...
    $alreadyIn = $settings["hooks"]["Stop"] | Where-Object {
        $_.hooks | Where-Object { $_.command -like "*mempal_save_hook*" }
    }
...
try {
    if (Get-ScheduledTask -TaskName "MemPalaceSync" -ErrorAction SilentlyContinue) {
        Write-Ok "Task 'MemPalaceSync' already registered"
    } else {
        ...
        Register-ScheduledTask -TaskName "MemPalaceSync" `
            -Action $action -Trigger $trigger -Settings $ts -RunLevel Highest -Force | Out-Null
        Write-Ok "Task 'MemPalaceSync' registered (runs every hour)"
    }
} catch {
...
if ($SkipVllm) {
    ...
    try {
        if (Get-ScheduledTask -TaskName "MemPalaceVLLM" -ErrorAction SilentlyContinue) {
            Write-Ok "Task 'MemPalaceVLLM' already registered"
        } else {
            ...
            Register-ScheduledTask -TaskName "MemPalaceVLLM" `
                -Action $action -Trigger $trigger -Settings $ts -RunLevel Highest -Force | Out-Null
            Write-Ok "Task 'MemPalaceVLLM' registered (starts vLLM at each login)"
        }
    } catch {
...
if (-not (Test-Path $palaceDb)) {
    Write-Skip "No palace at $palaceDb -- backfill skipped (run after mempalace mine)"
...
Write-Host ""
Write-Host "-------------------------------------------" -ForegroundColor White
Write-Host "  MemPalace setup complete!" -ForegroundColor Green
Write-Host ""
Write-Host "  Next steps:"
Write-Host "    1. Restart Claude Code  (hook takes effect)"
Write-Host "    2. claude mcp add mempalace -- python -m mempalace.mcp_server"
Write-Host "    3. Call mempalace_status in your first conversation"
if (-not $SkipVllm) {
