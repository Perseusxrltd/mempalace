...
def run_benchmark():
    ...
    report.append("This report compares the baseline Semantic Retrieval (ChromaDB) with the Hybrid Retrieval Protocol (FTS5 + RRF).")
    report.append("")
    report.append("## Summary Metrics")
    report.append(f"| Metric | Vector (Baseline) | Hybrid (Fused) | Improvement |")
    report.append(f"|---|---|---|---|")
    report.append(f"| MRR (Mean Reciprocal Rank) | {v_mrr:.4f} | {h_mrr:.4f} | +{((h_mrr/v_mrr)-1)*100 if v_mrr>0 else 100:.1f}% |")
    report.append(f"| Hit@1 Accuracy | {v_hit1*100:.1f}% | {h_hit1*100:.1f}% | +{(h_hit1-v_hit1)*100:.1f}% |")
    report.append("")
